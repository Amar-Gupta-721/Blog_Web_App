...
function PostForm({ post }) {
  ...
  const submit = async (data) => {
    try {
      ...
      // You can also handle errors by showing a message to the user
    }
  };
  ...
  return (
    <form 
      ...
      style={{ pointerEvents: errors.image ? 'none' : 'auto' }} // Prevent accidental blocking of form submission
    >
      <div className="w-full xl:w-2/3 lg:w-2/3 md:w-2/3 px-2 mb-3">
        <Input
          label="Title :"
      ...
      <div className="w-full xl:w-1/3 lg:w-1/3 md:w-1/3 px-2">
        ...
        <Button 
          ...
          type="submit" 
          className={`w-full bg-blue-500 hover:bg-blue-900 active:bg-red-600 ${post ? "bg-green-500" : undefined}`} 
        />
      </div>
    </form>
  );
