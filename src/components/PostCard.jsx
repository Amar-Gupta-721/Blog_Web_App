          ...
          <div className='flex shadow-xl overflow-auto hover:shadow-md duration-200 ease-in-out hover:scale-105 rounded-xl max-h-48 lg:max-h-64 lg:min-h-56 '>
            ...
            <div className="flex items-center text-neutral-50 text-sm lg:text-xl md:text-lg sm:text-sm">
              ...
              <Link to={`/post/${post.$id}`}>
                <h2 className='py-2 font-semibold text-neutral-50 capitalize truncate text-md lg:text-4xl md:text-3xl sm:text-2xl'>{post.title}</h2>
              </Link>
              <h4 className='text-neutral-50 text-sm lg:text-xl md:text-lg sm:text-sm'>By : {post?.userName || "User"}</h4>  {/*change (post?.userName || "user") by post.userName */}
              <Link to={`/post/${post.$id}`}>
                <Button
                children={"Read More"}
                className="text-xs text-neutral-800 bg-white md:text-sm lg:text-base xl:text-lg mt-2 xl:mt-3 lg:mt-3 md:mt-3 hover:bg-blue-400"
              />
              </Link>
            </div>
          </div>
